...
def contrast_lines(test_code):
    ...
    data = common.load_data(test_code)[0]
...
if __name__=="__main__":
    ...
    if symbol == 'Generic.Data':
        ...
        for csv_file in csv_files:
            if args.begin_code != "":
                if os.path.basename(csv_file).rsplit(".", 1)[0] != args.begin_code:
                    continue
                else:
                    args.begin_code = ""
    ...
    #             data = common.import_csv(ts_code, None)
    #             # data = common.data_queue.get()
        ...
        for epoch in range(0,common.EPOCH):
            ...
            for index, batch_data in enumerate(train_dataloader):
                if batch_data is None:
                    continue
                _train_dataloader=common.DataLoaderX(dataset=batch_data,batch_size=common.BATCH_SIZE,shuffle=False,drop_last=True, num_workers=8, pin_memory=True)
