...
# 运行时确保可以导入 stock_prediction 包（当直接脚本运行时）
...
def main(opt):
    ...
    pretrained_model = BertModel.from_pretrained(bert_data_path + '/base_model/bert-base-chinese',
                                                 cache_dir=bert_data_path + '/model/')
    model = Bert_Model(pretrained_model, opt)
    if os.path.exists(bert_data_path + '/model/bert_model.pth'):
        model.load_state_dict(torch.load(bert_data_path + '/model/bert_model.pth', map_location='cpu'))
...
def train(model, dataset, criterion, optimizer, opt, scheduler):
    ...
    for i, (input_ids, attention_mask, token_type_ids, labels) in enumerate(loader_train):
        ...
        iter_bar.update(1)
        iter_bar.set_description("loss: %.2e mean: %.2f acc: %.2f%%" % (
            loss.item(), total_loss_num / train_num, total_acc_num / train_num * 100))
        scheduler.step()
        if i % max(1, int(len(loader_train) / 10)) == 0:
            torch.save(model.state_dict(), bert_data_path + '/model/bert_model.pth')
...
def test(model, dataset, opt):
    ...
    test_acc = correct_num / max(1, test_num)
    if test_acc > test_best_acc:
        test_best_acc = test_acc
        torch.save(model.state_dict(), bert_data_path + '/model/bert_model_test_best.pth')
...
def collate_fn(data):
    ...
    token = BertTokenizer.from_pretrained(bert_data_path + '/base_model/bert-base-chinese',
                                          cache_dir=bert_data_path + '/model/')
