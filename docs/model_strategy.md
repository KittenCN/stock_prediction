...
- **评估指标**：RMSE、MAE、分位覆盖率、方向准确率，可补充收益相关指标（加权 RMSE、Calmar 比率等）。
...
- **实施步骤**：
  1. 特征工程：将原始指标按价格/波动/成交量分组，输入变量选择模块；外生变量可提供给 V-SSM。
  2. 模型训练：分别训练 PTFT 与 V-SSM，关注 RMSE、分位损失、regime 分类准确率等指标。
  3. 融合校准：在验证集上拟合线性组合或轻量模型，将状态概率用于校准分位区间。
  4. 推理部署：实时输入历史窗口 → PTFT 输出分布参数 → V-SSM 给出状态概率 → 融合层产出最终预测与风险提示。
...
| 优先级 | 改进方向 | 关键动作 | 预期收益 |
| ------ | -------- | -------- | -------- |
| 高 | **收益率序列建模** | 在 `data_preprocess.py` 中生成对数收益率/标准化差分，训练时允许选择收益率目标；同步新增回测基准 | 缓解价格序列非平稳性，提升趋势感知 |
