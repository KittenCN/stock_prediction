...
getdata_parser = argparse.Argumentgetdata_parser()
getdata_parser.add_argument('--code', default="", type=str, help="code")
getdata_parser.add_argument('--api', default="akshare", type=str, help="api-interface, tushare, akshare or yfinance")
getdata_parser.add_argument('--adjust', default="hfq", type=str, help="adjust: none or qfq or hfq, Note if you have permission")
getdata_args = getdata_parser.parse_getdata_args()

if getdata_args.api == "tushare":
    api_token = ""
...
def get_stock_data(ts_code="", save=True, start_code="", save_path=""):
    if getdata_args.api == "tushare":
        ...
        with lock:
            adjust = ""
            if getdata_args.adjust != "":
                adjust = "_" + getdata_args.adjust
        ...
        with lock:
            ...
            for code in stock_list:
                try:
                    df = ak.stock_zh_a_hist(symbol=code, period="daily", end_date=enddate, adjust=getdata_args.adjust)
                ...
                if save:
                    ...
                    if not df.empty:
                        ...
                        stock_data_queue.put(NoneDataFrame)
                        return None
    elif getdata_args.api == "yfinance":
        auto_adjust = False
        back_adjust = False
        if getdata_args.adjust == "":
            auto_adjust = False
            back_adjust = False
        elif getdata_args.adjust == "qfq":
            auto_adjust = True
            back_adjust = False
        elif getdata_args.adjust == "hfq":
            auto_adjust = True
            back_adjust = True
        stock_list = ts_code
        if save:
...
if __name__ == "__main__":
    ...
    #     os.mkdir(daily_path)
    yfi_ticker = ['DAX', 'IBM']
    if getdata_args.api == "yfinance":
        assert len(yfi_ticker) > 0, "Please input ticker"
        get_stock_data(yfi_ticker, save=True, save_path=daily_path)
    else:
        if getdata_args.code != "":
            get_stock_data(getdata_args.code, save=True, save_path=daily_path)
        else:
